...
def buscar_imagem_google_pw(page, query):
    ...
    query_buscada = f"{query} vitrine loja oficial site grande"
    ...
    try:
        ...
        for idx in range(min(15, len(imgs))):
             ...
             if src and ("http" in src.lower() or "data:image/jpeg;base64" in src.lower() or "data:image/png;base64" in src.lower()):
                 if "branding/googlelogo" not in src.lower() and "gstatic.com/images" not in src.lower() and len(src) > 100:
...
def process_js_pw():
    print(f"Iniciando Scraper Headless para Substituir Imagens Base64 ou de Baixa Resolucao!")
    ...
    with sync_playwright() as p:
        ...
        for b_idx, bloco in enumerate(blocos):
            # Target images starting with data:image (base64) or placeholders or empty
            if "data:image" in bloco or "'imagens/logo.png'" in bloco or '"imagens/logo.png"' in bloco or "img: ''" in bloco or "wixstatic" in bloco:
    ...
    print(f"\n============================================\nVarredura Concluida. {count_mod} imagens ricas injetadas via PW!\n============================================")
